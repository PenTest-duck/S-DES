...
if mode == "c":
    iv = input("Optionally input the 8-bit IV (leave blank for randomisation): ")
    if iv == '':
        for i in range(8):
...
def CBC_Block(blocks):
    output = []
    if method == "e":
        iv_block = XOR(blocks[0], iv_bits)
        blocks[0] = iv_block
        output += Block(blocks[0]) 
        next_iv = output
        for i in range(1, len(blocks)):
            block_out = Block(XOR(blocks[i], next_iv))
            next_iv = block_out
            output += block_out
        return output
    elif method == "d":
        for i in range(len(blocks) - 1, 0, -1):
            next_iv = blocks[i - 1]
            output = XOR(Block(blocks[i]), next_iv) + output
        output = XOR(Block(blocks[0]), iv_bits) + output
    return output

output = ""
if mode == "e":
    for i in range(len(blocks)):
        output += toString(Block(blocks[i]))
elif mode == "c":
    output += toString(CBC_Block(blocks))
